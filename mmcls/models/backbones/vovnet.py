...
VoVNet19_slim_eSE = {
    ...
    'layer_per_block': 3,
    'block_per_stage': [1, 1, 1, 1],
    #  'block_per_stage': [2, 2, 2, 1],
    #  'use_dcn': [True, True, True, True],
    #  'use_dcn': [True, True, True, True],
...
def conv3x3(
    in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=3, padding=1, use_dcn=False
):
    if use_dcn:
        """3x3 convolution with padding"""
        return [
...
class DeformConvPack(DeformConv):
    ...
    def forward(self, x):
        ...
        return deform_conv2d(x, offset, self.weight, bias=None, stride=self.stride, padding=self.padding,
...
class _OSA_module(nn.Module):
    def __init__(
        self, in_ch, stage_ch, concat_ch, layer_per_block, module_name, SE=False, identity=False, depthwise=False, use_dcn=False
    ):
        ...
        for i in range(layer_per_block):
            if self.depthwise:
                ...
                use_dcn = use_dcn and i == 0
                self.layers.append(
                    nn.Sequential(OrderedDict(conv3x3(in_channel, stage_ch, module_name, i, use_dcn=use_dcn)))
                )
            in_channel = stage_ch
...
class _OSA_stage(nn.Sequential):
    def __init__(
        ...
        self.add_module(
            module_name, _OSA_module(in_ch, stage_ch, concat_ch, layer_per_block, module_name, SE, depthwise=depthwise, use_dcn=use_dcn)
        )
...
class VoVNet(BaseBackbone):
    def __init__(self, spec, num_stages, use_dcn):
        """
        Args:
        ...
        input_ch = 3
        assert (len(use_dcn) == num_stages + 2)

        stage_specs = _STAGE_SPECS[spec]
        ...
        stem = conv3x3(input_ch, stem_ch[0], "stem", "1", 2, use_dcn=use_dcn[0])
        stem += conv_type(stem_ch[0], stem_ch[1], "stem", "2", 1)
        stem += conv_type(stem_ch[1], stem_ch[2], "stem", "3", 2, use_dcn=use_dcn[1])
